...
export default function WinnerAnnouncement({ completedTasks, children }) {

    const weeklyWinner = useMemo(() => {
        if (!isTimeToAnnounceWinner() || !completedTasks || !children) return null;

        const { start, end } = getPreviousWeekRange();
        
        const prevWeekCompleted = completedTasks.filter(ct => {
        ...
        const scores = children.map(child => {
            const childCompleted = prevWeekCompleted.filter(ct => ct.child_id === child.id);
