...
const ScheduleCell = ({ day, period, schedule, subjects, teachers, assignments, selectedChildId, onSave, onDelete, isReadOnly }) => {
    ...
    const getSubjectInfo = useCallback((subjectId) => {
        if (!subjectId || !subjects) return { name: '', color: '#F3F4F6', location: '' };
        ...
        return (
             <div className="relative h-full min-h-[60px]">
                <Select defaultValue={schedule?.subject || ''} onValueChange={handleSubjectChange} onOpenChange={(isOpen) => !isOpen && setIsEditing(false)} defaultOpen={true}>
                    ...
                    <SelectContent>
                        <SelectItem value={null}>-- הסר שיעור --</SelectItem>
...
const MobileScheduleView = ({ scheduleGrid, subjects, teachers, assignments, child, onSave, onDelete, isReadOnly }) => (
...
export default function ScheduleManager({ child, schedules, subjects, teachers, assignments, onSave, onDelete, isReadOnly }) {
    ...
    return (
        <Card>
            <CardHeader><CardTitle>מערכת השעות של {child.hebrew_name}</CardTitle></CardHeader>
            <CardContent>
                {isMobile ? (
                    <MobileScheduleView 
